...
impl Apu {
    ...
    pub fn tick(&mut self, cycles: u32) {
        for _ in 0..cycles {
            ...
            if self.sample_clock % ((CPU_CLOCK / (SAMPLE_RATE as usize)) as u32) == 0 {
...
impl Addressable for ChannelOne {
    #[inline]
    fn read(&self, addr: u16) -> u8 {
        match addr {
            0xFF10 => (self.sweep_period << 4) | (if self.sweep_is_decrementing { 0x08 } else { 0x00 }) | self.sweep_amount | 0x80,

            0xFF11 => (self.duty_pattern << 6) | 0b0011_1111,

            0xFF12 => (self.initial_volume << 4) | (if self.is_incrementing { 0x08 } else { 0x00 }) | self.period,
...
impl Addressable for ChannelTwo {
    #[inline]
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xFF17 => (self.initial_volume << 4) | (if self.is_incrementing { 0x08 } else { 0x00 }) | self.period,
...
impl Addressable for ChannelThree {
    #[inline]
    fn read(&self, addr: u16) -> u8 {
        match addr {
...
impl Addressable for ChannelFour {
    #[inline]
    fn read(&self, addr: u16) -> u8 {
        match addr {
            // NR40 does not exist.
            ...
            0xFF21 => (self.initial_volume << 4) | (if self.is_incrementing { 0x08 } else { 0x00 }) | self.period,
...
impl Addressable for Apu {
    ...
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xFF10..=0xFF14 => self.channel_one.read(addr),

            // Channel 2 IO registers.
            0xFF15..=0xFF19 => self.channel_two.read(addr),

            // Channel 3 IO registers + Wave RAM.
            0xFF1A..=0xFF1E | 0xFF30..=0xFF3F => self.channel_three.read(addr),

            // Channel 4 IO registers.
            0xFF1F..=0xFF23 => self.channel_four.read(addr),
    ...
    #[inline]
    fn write(&mut self, addr: u16, value: u8) {
        if !(self.apu_enabled || addr == 0xFF26 || (0xFF30..=0xFF3F).contains(&addr) || [0xFF11, 0xFF16, 0xFF1B, 0xFF20].contains(&addr)) {
            return;
        }
        ...
        match addr {
            ...
            // Channel 1 IO registers.
            0xFF10..=0xFF14 => self.channel_one.write(addr, value),

            // Channel 2 IO registers.
            0xFF15..=0xFF19 => self.channel_two.write(addr, value),

            // Channel 3 IO registers + Wave RAM.
            0xFF1A..=0xFF1E | 0xFF30..=0xFF3F => self.channel_three.write(addr, value),

            // Channel 4 IO registers.
            0xFF1F..=0xFF23 => self.channel_four.write(addr, value),
