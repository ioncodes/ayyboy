...
mod tests {
    ...
    fn test_cpu(input: &str) {
        let tests: serde_json::Value = serde_json::from_str(&input).unwrap();
        ...
        for test in tests.as_array().unwrap() {
            ...
            for value in ram {
                ...
                assert_eq!(mmu.read(addr), value, "Comparison with RAM failed");
            }
        }
    }
}
